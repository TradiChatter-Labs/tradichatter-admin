...
import { Smartphone, Send, Users } from 'lucide-react';
...
export default function PushNotifications() {
  ...
  const [form, setForm] = useState({ title: '', body: '', user_type: '', active_mode: '' });
  ...
  const fetchCampaigns = async () => {
    try {
      const res = await fetch(`${COMMS_URL}/api/push/campaigns`, { headers: { 'X-API-Key': COMMS_KEY } });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await fetch(`${COMMS_URL}/api/push/campaign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': COMMS_KEY },
        body: JSON.stringify({ title: form.title, body: form.body, segment_filters })
      });
      const json = await res.json();
      setResult(json);
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Send push notifications to app users via Expo</p>
        </div>
        ...
        {tab === 'compose' && (
          <div className="bg-white rounded-lg shadow p-6 space-y-4">
            <div>
              ...
              <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full border rounded-md px-3 py-2" placeholder="Notification title" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Body</label>
              <textarea value={form.body} onChange={e => setForm({...form, body: e.target.value})} className="w-full border rounded-md px-3 py-2" rows="3" placeholder="Notification message..." />
            </div>
            ...
            <button onClick={handleSend} disabled={sending} className="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 disabled:bg-gray-400 flex items-center">
              <Send className="h-4 w-4 mr-2" />{sending ? 'Sending...' : 'Send Push Campaign'}
            </button>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {campaigns.length === 0 ? (
                  <tr><td colSpan="5" className="px-6 py-8 text-center text-gray-500">No campaigns yet</td></tr>
                ) : campaigns.map(c => (
                  <tr key={c.id}>
                    <td className="px-6 py-4"><div className="text-sm font-medium">{c.title}</div><div className="text-xs text-gray-500">{c.body?.slice(0, 50)}</div></td>
