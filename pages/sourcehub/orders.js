...
import { ShoppingCart, RefreshCw, DollarSign, Clock, CheckCircle, Truck } from 'lucide-react';

export default function SourceHubOrders() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sourcehub/orders?${params}`);
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">SourceHub — Wholesale Orders</h1>
        <p className="mt-1 text-sm text-gray-600">Monitor B2B wholesale orders across all suppliers.</p>
      </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-500">No orders found. Ensure SourceHub service is running.</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-200">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{order.buyer_name || order.buyer_id?.slice(0, 8)}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{order.supplier_name || order.supplier_id?.slice(0, 8)}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{order.escrow_status || '—'}</td>
